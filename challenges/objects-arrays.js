...
const contactInfo = [];
let nameAndEmail = graduates.forEach(
    nameEmailCallback = (
        (item,idex,arr) => {
...
//const unisWithUni = [];
// const unisWithUni = graduates.map(
//     (item) => {
...
//     }
// )
const unisWithUni = graduates.filter(
    (item) => {
...
let nameJoin = zooAnimals.forEach(
    nameConcatCallback = (
        (item,index,arr) => {
...
lowCaseAnimalNames = zooAnimals.map(
    (item) => {
...
*/
let populationTotal = zooAnimals.reduce((total,item) => {
